...
% 3: normal vector
% 5...: Ex Ey Ez Hx Hy Hz at each (position, time step). 
% Positions are ordered in the way that abides to the local reference frame 
% of each face (x1, x2, x3). where x3 is the normal direction In this way,
% x1 is incremented first, then x2, then x3
...
for i = 1 : 6
    ...
    E{i} = E{i} .* reshape(exp(-1j * 2 * pi * t' * ft'), 1, 1, [], slice_len);
    E{i} = reshape(sum(E{i}, 3), 3, [], slice_len) ./ reshape(ref, 1, 1, slice_len);
    
    H{i} = H{i} .* reshape(exp(-1j * 2 * pi * t' * ft'), 1, 1, [], slice_len);
    H{i} = reshape(sum(H{i}, 3), 3, [], slice_len) ./ reshape(ref, 1, 1, slice_len);

    % get surface currents
    norm_mat = [0 -norm_vec(3) norm_vec(2);
        norm_vec(3) 0  -norm_vec(1);
        -norm_vec(2) norm_vec(1) 0];
    
    J = norm_mat * reshape(H{i}, 3, []);    %(3 x 3) * (3 x (pos x slice_len))
    M = -norm_mat * reshape(E{i}, 3, []);
    J = reshape(J, 3, [], slice_len);       %3 x pos x slice_len
    M = reshape(M, 3, [], slice_len);
