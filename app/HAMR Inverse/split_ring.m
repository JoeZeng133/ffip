...
r1 = make_rect([0, 0], [c, b]);
r2 = make_rect([0, 0], [c + 2 * d, b + 2 * d]);
r3 = make_rect([0, -b/2 - d/2], [a, d]);

tmp = subtract(r2, r1);
au_shape = subtract(tmp, r3);

bbox_p1 = [-100e-9, -100e-9];
bbox_p2 = [100e-9, 100e-9];
bbox = make_rect((bbox_p1 + bbox_p2) / 2, (bbox_p2 - bbox_p1));

figure
plot(scale(bbox, 1e9)), hold on
plot(scale(au_shape, 1e9)), hold off
...
% gold aperture
[au_rho, au_x, au_y, au_z, au_p1, au_p2, au_cell] = make_inhom(au_shape, 100e-9, bbox_p1, bbox_p2, dx);
au_center = [center(1); center(2); 55e-9];
au_geom = struct('type', 'inhom', 'medium_idx1', 1, 'medium_idx2', 0, 'filename', 'au_geom.in',...
    'dim', size(au_x), 'position', au_p1 + au_center, 'dx', au_cell, 'rho', au_rho * 1);

%FePt layer
fe_center = [center(1) center(2) 115e-9];
