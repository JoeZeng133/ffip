...
sim_forward = ffip.Simulation(
    ...
    sources=sources,
    pmls=pmls,
    input_file='disk_vary_r_forward_input.h5',
    fields_output_file='disk_vary_r_forward_output.h5',
...
sim_adjoint = ffip.Simulation(
    ...
    resolution=1/dx,
    pmls=pmls,
    input_file='disk_vary_r_adjoint_input.h5',
    fields_output_file='disk_vary_r_adjoint_output.h5',
    progress_interval=1e10
)
...
for r in r_list:
    ...
    sim_forward.run(stop_condition=stop_condition, np=2, skip=True, pop=True)
    ...
    sim_adjoint.run(stop_condition=stop_condition, np=2, skip=True, pop=True)
