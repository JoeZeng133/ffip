...
shift = np.angle(G)/(2*pi*ft)
src_func = ffip.Gaussian1(fs, cutoff=4, start_time=0.5/ft - shift)
t = np.arange(src_func.start_time, src_func.end_time, dt)
ref_signal = src_func(t)
ref_signal_fft = np.sum(ref_signal * np.exp(-2j*pi*ft*(t + shift)), axis=0)
...
sim = ffip.Simulation(
    ...
    resolution=1/dx,
    courant=sc,
    input_file='dipole_input.h5',
    fields_output_file='dipole_output.h5',
...
sim.run(stop_condition=ffip.run_until_time(src_func.end_time * 10))

#%% read fields
fex = ex_dft.get_interpolant(method='linear',bounds_error=False, fill_value=None)
fey = ey_dft.get_interpolant(method='linear',bounds_error=False, fill_value=None)
fez = ez_dft.get_interpolant(method='linear',bounds_error=False, fill_value=None)
