...
class Inhom_Source:
    def __init__(self, function, amplitude=None, frequency=1, center=Vector3(), size=Vector3(), dim=Vector3(), field_component='Ex', suffix='0'):
        ...
        if callable(amplitude):
            self.amplitude = amplitude(np.stack(np.meshgrid(
    ...
    def amplitude(self, val):
        if not cmp_shape(self.shape, val.shape):
            raise ValueError(
                'amplitude shape is not compatibale with source dimension')
    ...
    def dump(self, file):
        file.create_dataset(self.amplitude_dataset_name,
                            data=np.abs(self.amplitude.ravel()))
        file.create_dataset(self.phase_dataset_name,
                            data=np.angle(self.amplitude.ravel()))
